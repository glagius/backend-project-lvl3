  ...
  describe('Test how app works with success http requests', () => {
    ...
    test('App saves files into created directory', async () => {
      ...
      const promises = assetsPaths.map((asset) => {
        ...
        const fixture = fixturePaths.find((el) => el.includes(assetname.split('-').reverse()[0]));
        return fsPromises.readFile(path.resolve(`${dirname}/fixtures/${fixture}`));
      });

      const contents = await Promise.all(promises);
      const fixtures = fixturePaths.map((asset) => fsPromises.readFile(path.resolve(`${dirname}/fixtures/${asset}`)));
